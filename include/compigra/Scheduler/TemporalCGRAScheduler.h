...
namespace compigra {
/// Class for basic block ILP scheduler. BasicBlockILPScheduler builds an ILP
...
/// Strategy to handle BasicBlockILPScheduler failure, if Abort, abort the
...
/// BasicBlockILPScheduler class to build an ILP model to schedule the
/// placement and execution time for each operation in the basic block.
class BasicBlockILPScheduler : public CGRAKernelScheduler<ScheduleUnitBB> {
public:
  BasicBlockILPScheduler(unsigned maxReg, unsigned nRow, unsigned nCol,
